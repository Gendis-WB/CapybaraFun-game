...
document.addEventListener('keydown', (e) => {
  if ((e.code === 'Space' || e.code === 'ArrowUp') && canJump) {
    capy.dy = -28;
    canJump = false;
    point++;
...
// === SPAWN OBSTACLE TIAP 1.5 DETIK ===
setInterval(() => {
  if (!isPaused) spawnObstacle();
}, 1500);

// === BGM MULAI SAAT INTERAKSI ===
...
function endGame() {
  ...
  localStorage.setItem('lastPoint', point);

  let leaderboard = JSON.parse(localStorage.getItem('leaderboard')) || [];
  leaderboard.push({ name: playerName, point, score, time });
  leaderboard.sort((a, b) => b.point - a.point || b.score - a.score || a.time - b.time);
  leaderboard = leaderboard.slice(0, 10);
  localStorage.setItem('leaderboard', JSON.stringify(leaderboard));

  window.location.href = 'AkhirVer2.html';
}
